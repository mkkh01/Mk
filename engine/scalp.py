...
class ScalpMonitor:
    ...
    async def evaluate(self, symbol: str) -> ScalpDecision:
        ...
        trend_scores = [
            float(context.get("strength", 0.0)),
            float(bias.get("strength", 0.0)),
            float(setup.get("strength", 0.0)),
            float(trigger.get("strength", 0.0)),
        ]
        ...
        if decision.confidence < SCALP_MIN_CONFIDENCE:
            ...
            return decision
        if SCALP_TARGET_PCT - SCALP_STOP_PCT < SCALP_MIN_NET_EDGE_PCT:
